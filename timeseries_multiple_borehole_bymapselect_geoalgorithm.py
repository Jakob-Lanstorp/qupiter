...
from processing.core.parameters import ParameterExtent, ParameterTableField, \
    ParameterFile, ParameterString, ParameterVector, ParameterBoolean, ParameterSelection, ParameterTable
...
class TimeSeriesMultipleBoreholeByMapSelectGeoAlgorithm(GeoAlgorithm):
    ...
    def defineCharacteristics(self):
        ...
        self.addParameter(ParameterVector(
            self.INPUT_TABLE,
            self.tr(u'Vælg tabel med valgte boringer'),
    ...
    def shortHelp(self):
        return self._formatHelp(u'''Tidsserieplot et stof flere boringer valgt i kort<p>Boringer til tidsserieplot skal være valgt i kort<p>''')
    ...
    def processAlgorithm(self, progress):
        ...
        qgis = JupiterQGIS()
        selected_boreholeno = qgis.get_selected_boreholeno(input_table, boreholeno_field)

        if selected_boreholeno is None:
            progress.setText(u'<b>Der er ikke udvalgt nogle boringer<br></b><br>')
            return
        ...
        if 'test_mode' == 'XXXXtest_mode':
            ...
            result, not_found = mapplot.timeseries_onecompound_multipleborehole(
                selected_boreholeno, compounds, date_from, date_to, progress=progress)

        if result and len(not_found) == 0:
            progress.setText(u'<b>Tidsserie for "{}" i boringer: "{}" er oprettet.<br></b><br>'.format(
                compounds.replace(';', ','), ', '.join(selected_boreholeno)))
        elif result and len(not_found) > 0:
            progress.setText(u'<b>Tidsserie for "{}" i boringer: "{}" er oprettet.<br>Boringer hvor stof ikke er fundet er: "{}"</b><br>'.format(
                compounds.replace(';', ','), selected_boreholeno, ', '.join(not_found)))
        else:
            progress.setText(u'<b>NO DATE: Boringer: "{}" har ingen analyser for {} i tidsperioden {} til {}.<br></b><br>'.
                             format(selected_boreholeno, compounds.replace(';', ','), date_from, date_to))

        progress.setPercentage(100)

    def test_onecompound_multipleborehole_bymapselect(self):
